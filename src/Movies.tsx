...
import {
  ...
  const Movies = () => {
    ...
    const fetchData=()=>{
        ...
        fetch('https://api.themoviedb.org/3/movie/'+category+'?api_key=369f86b614006cd7c965f3f582810609&language=en-US&page=1')
        ...
        let genreArr:string[]=[];
          ...
          array.push(detailsArr.concat(x.id,imgURL+x.poster_path,x.title,x.vote_average+'/10', genreStr));
          })
          setMovies(array);
    ...
    backgroundColor: 'transparent'
      ...
      return (
        ...
        <Nav className='categoriesNav'>
            ...
            <Nav.Item><Nav.Link style={category=='now_playing'?activeStyle:passiveStyle} onClick={handleNowPlaying}>Now Playing</Nav.Link></Nav.Item>
            <Nav.Item><Nav.Link style={category=='upcoming'?activeStyle:passiveStyle} onClick={handleUpcoming}>Upcoming</Nav.Link></Nav.Item>
            <Nav.Item><Nav.Link style={category=='top_rated'?activeStyle:passiveStyle} onClick={handleTopRated}>Top Rated</Nav.Link></Nav.Item>
        </Nav>
        <div className='movies-tv-grid-container'><h4>{category=='popular'?'Popular':category=='upcoming'?'Upcoming'
        :category=='top_rated'?'Top Rated': 'Now Playing'} Movies</h4>
      <div className='movies-tv-grid' >
        {movies.map((x:any)=> 
        <Link to={'/film/'+x[0]} className='movies-tv-link'><div className='movies-tv-card' >
