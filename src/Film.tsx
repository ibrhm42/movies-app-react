...
import { faCaretDown, faList, faSearch, faHeart, faBookmark, faCaretRight, faStar, faCircle, faArrowRight, faCalendar, faHome, faTimesCircle, faMale, faFemale } from '@fortawesome/free-solid-svg-icons';
...
const Film= ({match}: any)=> {
    
  const [contentType, setContentType]=useState('fuck u');
  const [id, setID]= useState('597');
  ...
  const updateFilm = (id:string) => {
    fetch('https://api.themoviedb.org/3/movie/'+id+'?api_key=369f86b614006cd7c965f3f582810609&language=en-US')
    .then(response => response.json()).then(response => {
      setTitle(response.title);
      setPoster(imgURL+response.poster_path);
      setCover(imgURL+response.backdrop_path)
      response.release_date && setYear(response.release_date.slice(0,4));
      response.genres[0] && setGenres(response.genres.map((x:any) => x.name).reduce((a:string,b:string)=> `${a}, ${b}`));
      let durationHour=Math.trunc(response.runtime/60)!=0 ? Math.trunc(response.runtime/60).toString()+'h ':'';
      let durationMin=(response.runtime%60)!=0 ? (response.runtime%60).toString()+'m': '';
      ...
      setOriginalName(response.original_title);
      ...
      //Update Cast & Directors 

      fetch('https://api.themoviedb.org/3/movie/'+id+'/credits?api_key=369f86b614006cd7c965f3f582810609')
    .then(response => response.json()).then(response => {
      let array:any=[];
      ...
      let arr:string[]=[];
      response.crew.map((x:any)=> {
        x.job=='Director' && arr.push(x.name);
      });
      setDirectors(arr);
    });
    ...
    fetch('https://api.themoviedb.org/3/movie/'+id+'/videos?api_key=369f86b614006cd7c965f3f582810609&language=en-US')
    .then(response => response.json()).then(response => {
    ...
    fetch('https://api.themoviedb.org/3/movie/'+id+'/images?api_key=369f86b614006cd7c965f3f582810609')
    .then(response => response.json()).then(response => {
  ...
  fetch('https://api.themoviedb.org/3/movie/'+id+'/recommendations?api_key=369f86b614006cd7c965f3f582810609&language=en-US&page=1')
  .then(response => response.json()).then(response => {
    if(response.results){
    let array:any=[];
    let detailsArr:string[]=[];
    response.results.map((x:any)=> {
      detailsArr=[];
      array.push(detailsArr.concat(x.title, x.vote_average, imgURL+x.backdrop_path, x.release_date, x.id))});
  ...
  fetch('https://api.themoviedb.org/3/movie/'+id+'/external_ids?api_key=369f86b614006cd7c965f3f582810609&language=en-US')
  .then(response => response.json()).then(response => {
  ...
  fetch('https://api.themoviedb.org/3/movie/'+id+'/keywords?api_key=369f86b614006cd7c965f3f582810609')
  .then(response => response.json()).then(response => {
    setKeywords(prev=> prev.concat(response.keywords.map((x:any)=> x.name)));
  })

  }

  useEffect(()=> {
    updateFilm(match.params.id);
  },[])
  ...
  setFilmSearch([]);
    ...
    function searchClick(event:any){
      console.log(event.currentTarget.id.toString());
      setID(event.currentTarget.id.toString());
    }
...
<Nav.Link href={'/imdb/tv/'+TVSearch[i][4]} >{TVSearch[i][2]}</Nav.Link>
  ...
  <Nav.Item>
    ...
    <div className='directors'>
      <h6>{directors.map((x:string)=> x).join(', ')}</h6>
      {directors.length>1 ? <span>Directors</span> : <span>Director</span>}
    </div>
                  </div> 
                  </div>
                  </div>
  ...
  {trailer.length>0 && <iframe className='media-iframes' src={trailer}    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" 
    ...
    {recommendations.length>0 && <div className='recommendations-section'>
      ...
      {recommendations.map((x:any)=> <a href={'/imdb/film/'+x[4]}><div className='reccomendations-tile'>
